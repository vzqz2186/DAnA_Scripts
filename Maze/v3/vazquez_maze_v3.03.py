"""
      ...
      Execute via: >>python vazquez_maze_v3.03.py
...
# ---------------------------------------------------------------------------80
# Allows a player to traverse the maze. (A2)
def solve(maze, height, width):

    COL = 1
    ROW = 1
    # Starting position is set to the player's symbol.
    maze[COL][ROW] = 'O'
    
    # Loop allows the game to continue as long as finish vertex != 'O'. (2.a)
    while(maze[height][width] != 'O'):

        # Prints maze.
        display(maze)

        # Control map.
        print("W: Up | A: Left | S: Down | D: Right | E: Exit")
        # Direction input. (2.a
        path = input("Enter direction (W, A, S, D) or exit: ")
        print("\n")

        # Depending on the player wants to go, the following code changes the
        # player's previous location to a 0, a V (if the vertex has a 0), or 
        # bounces if there's a wall.
        
        # Allows the player to move DOWN. 
        if(path == 'S' or path == 's'):
            if(maze[COL+1][ROW] == ' ' or maze[COL+1][ROW] == 'V'): # (2.b)
                maze[COL][ROW] = 0
                COL += 1
                maze[COL][ROW] = 'O'
            if(maze[COL+1][ROW] == 0): # (2.c)
                maze[COL][ROW] = 'V'
                COL += 1
                maze[COL][ROW] = 'O'
            if(maze[COL+1][ROW] == 'X'):
        ...
        if(path == 'D' or path == 'd'):
            if(maze[COL][ROW+1] == ' ' or maze[COL][ROW+1] == 'V'): # (2.b)
                maze[COL][ROW] = 0
                ROW += 1
                maze[COL][ROW] = 'O'
            if(maze[COL][ROW+1] == 0): # (2.c)
                maze[COL][ROW] = 'V'
                ROW += 1
                maze[COL][ROW] = 'O'
            if(maze[COL][ROW+1] == 'X'):
        ...
        if(path == 'E' or path == 'e'):
            break
        # If path != any of the allowed options, it just passes.
        else:
            pass

    # If the finish vertex  == 'O', the game ends. (2.d)
    if(maze[height][width] == 'O'):
        display(maze)
        print("\nMaze solved. Congratulations!")
